...
function blopmatch_estimate()
{
    ...
    for (group = 1; group <= s("Ngroups"); group++) {
        ...
        blopmatch_get_var_weights(Xsub, Nsub, W, B)

        // Get ate/ate variance estimate
        vars[group] = blopmatch_get_var(Ysub, Nsub, W, B, C, Yb, pnts[group])
    }
...
)
{
    ...
    if ((K + 1) > N) _error("Insufficient observations")
    ...
    muID = (0 + 1) :: (1 * K)             // Indices de b relativos a mu (inic)
    nuID = (K + 1) :: (2 * K)             // Indices de b relativos a nu (inic)
    cri  = (x - X[., N]) :>  0            // Criterio de entrada a b para μ/ν
    muID = muID[selectindex(cri :== 1)]   // Si cri[i] >  0, μ[i] es basico
    nuID = nuID[selectindex(cri :== 0)]   // Si cri[i] <= 0, ν[i] es basico
...
)
{
    ...
    if (Q < (1 + K)) {
        ...
        // Method B (robust yet inefficient)
        else {
            col = 1
            rk0 = rank(P2.A[., B])
            while (rk0 < (1 + K)) {
